...
if __name__ == "__main__":
    ...
    if library.lower() in ["pytorch", "torch"]:
        ...
        # Pytorch TEST
        if use_feedback:
            arms = ['type_mutation', 'value_mutation', 'db_selection']
            bandit = MutationBandit(arms)
        ...
        for _ in range(each_api_run_times):
            ...
            if use_feedback:
                ...
                max_gain = max(py_cov_gain, c_cov_gain)
                if max_gain > 0:
                    norm_py_gain = py_cov_gain / max_gain
                    norm_c_gain = c_cov_gain / max_gain
                    reward = (norm_py_gain + norm_c_gain) / 2
                else:
                    reward = 0
