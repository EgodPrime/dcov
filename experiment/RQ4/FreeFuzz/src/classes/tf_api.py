...
class TFAPI(API):
    ...
    def mutate2(self, chosen_arm):
        ...
        for _ in range(num_Mutation):
            arg_name = choice(list(self.args.keys()))
            arg = self.args[arg_name]
            if chosen_arm == 'type_mutation':
                arg.mutate_type()
            elif chosen_arm == 'value_mutation':
                arg.mutate_value()
            elif chosen_arm == 'db_selection':
                new_arg, success = TFDatabase.select_rand_over_db(
                    self.api, arg_name)
                if success:
                    new_arg = TFArgument.generate_arg_from_signature(
                        new_arg)
                    self.args[arg_name] = new_arg
